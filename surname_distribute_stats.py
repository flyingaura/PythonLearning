...
index_sur = []
others_sur = []
# sur_list_temp = sur_list
i = 0
while(i < len(sur_list)):
    number_sur = 1
    for j in range(i+1,len(sur_list)):
        if(sur_list[i][1] != '***'):
            if(sur_list[i][0] == sur_list[j][0]):
                number_sur = number_sur + 1
                sur_list[j][1] = '***'
    if(sur_list[i][1] == 'normal'):
        index_sur.append({sur_list[i][0]:number_sur})
    elif(sur_list[i][1] == 'others'):
        others_sur.append({sur_list[i][0]:number_sur})
    i = i + 1

# print(index_sur)
# 输出统计结果
person_total = len(person_list)
# 先做一下结果排序
istep = 0
while(istep < len(index_sur)):
    sorted_sur = index_sur[istep]
    sorted_index = istep
    for i in range(istep,len(index_sur)):
        if(list(sorted_sur.values())[0] < list(index_sur[i].values())[0]):
            sorted_sur = index_sur[i]
            sorted_index = i
    if(sorted_index != istep):
...
for sur_key in index_sur:
    ...
    print(list(sur_key.keys())[0],' : ',list(sur_key.values())[0],outstr,'  %f' % (per_values*100),'%')
if(others_sur != []):
    others_num = 0
    for others_key in others_sur:
        others_num = others_num + list(others_key.values())[0]
    per_values = float(others_num/person_total)
    print('others : %d ' % others_num,graphic_out(per_values),' %f' % (per_values*100),'%')
    print('others : ', end='')
    for others_key in others_sur:
        print('%s(%d)  ' % (list(others_key.keys())[0],list(others_key.values())[0]), end= '')
